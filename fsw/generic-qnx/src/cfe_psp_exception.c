...
void CFE_PSP_ExceptionSigHandler(int signo, siginfo_t *si, void *ctxt)
{
    ...
    if (Buffer != NULL)
    {
        ...
        /* Set exception context size */
        Buffer->context_size = sizeof(CFE_PSP_Exception_ContextDataEntry_t);
...
void CFE_PSP_AttachExceptions(void)
{
    ...
    /* Display exception context entry size */
    OS_printf("CFE_PSP: %s ==> Exception Context Size: %ld \n", __func__, sizeof(CFE_PSP_Exception_ContextDataEntry_t));
}
...
void CFE_PSP_SetDefaultExceptionEnvironment(void)
{
     ...
     * such as divide by zero and additionally SIGILL/SIGBUS/SIGSEGV.
     *
     * When debugging, remove the attach sig handler call and use the default signal handler
     * for the SIGILL/SIGBUS/SIGSEGV which will abort the program and generate a core file.
     */
    CFE_PSP_AttachSigHandler(SIGFPE);
    CFE_PSP_AttachSigHandler(SIGILL);
    CFE_PSP_AttachSigHandler(SIGBUS);
    CFE_PSP_AttachSigHandler(SIGSEGV);
}
...
int32 CFE_PSP_ExceptionGetSummary_Impl(const CFE_PSP_Exception_LogData_t *Buffer, char *ReasonBuf, uint32 ReasonSize)
{
    ...
    else if (Buffer->context_info.si.si_signo == SIGILL)
    {
        (void)snprintf(ReasonBuf, ReasonSize, "Caught SIGILL");
    }
    else if (Buffer->context_info.si.si_signo == SIGBUS)
    {
        (void)snprintf(ReasonBuf, ReasonSize, "Caught SIGBUS");
    }
    else if (Buffer->context_info.si.si_signo == SIGSEGV)
    {
        (void)snprintf(ReasonBuf, ReasonSize, "Caught SIGSEGV");
    }
    else
    {
        /*
