...
#if 0
        /* 
        ** VxWorks todo: VxWorks does not support backtrace - try to figure out the best exception
        ** strategy and implement it. At a minimum, capture the context and log it in the context data structure
        */
...
**
** An extension of CFE_PSP_ExceptionSigHandler that also
** kills the calling task. In a VxWorks RTP, this allows the
** OSAL cleanup to complete so the system can restart.
**
** This is required for handling events like Floating Point exceptions,
** where returning to the application would resume at the same instruction
** and re-trigger the exception, resulting in a loop.
**
*/
void CFE_PSP_ExceptionSigHandlerKill(int signo, siginfo_t *si, void *ctxt)
{
    int rc = -1;
...
#if 0
    /*
    ** VxWorks RTP todo: VxWorks does not support backtrace
    */
    void *Addr[1];
