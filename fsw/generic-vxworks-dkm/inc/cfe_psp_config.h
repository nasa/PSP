...
typedef struct
{
    OS_time_t  time_stamp;     /* time from OS_GetLocalTime */
 ...
 *
 * Chosen as the cache line size of the MCP750 processor (32 bytes)
 * such that the blocks will be cached more efficiently. (Might need
 * to be changed for generic support instead of PowerPC specific)
 */
