...
{
    CFE_PSP_ReservedMemoryMap.BootPtr->bsp_reset_type = reset_type;
}

uint32 UT_BootRecAdaptor_Get_SizeOfRecord(void)
{
    return sizeof(CFE_PSP_ReservedMemoryBootRecord_t);
}

void UT_BootRecAdaptor_Init(void *MemPtr)
{
    CFE_PSP_ReservedMemoryMap.BootPtr = MemPtr;
}